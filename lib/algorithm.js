...
function getPackageDependencies(name, version, repository) {
  var packageInfo = repository.get(name);
...
function resolvePackages(repository, rootDependencies, currentSolution, remainingToSelect) {
  ...
  for (var i = 0; i < depNames.length; i++) {
    ...
    for (var j = 0; j < versions.length; j++) {
      ...
      try {
        return resolvePackages(repository, getPackageDependencies(dep, version, repository), newSolution, newRemaining);
      } catch (e) {
